...
const Navbar = () => {
  ...
  // ---------------------------------------------------
  // LOAD NAVBAR LOGO FROM CENTRALIZED CMS (content_items)
  // ---------------------------------------------------
  useEffect(() => {
    const fetchLogo = async () => {
      try {
        const res = await fetch(`${API_BASE}/api/cms/navbar`);
        const data = await res.json();

        if (data?.logo) {
          setLogoUrl(`${API_BASE}/uploads/${data.logo}`);
        }
      } catch (err) {
